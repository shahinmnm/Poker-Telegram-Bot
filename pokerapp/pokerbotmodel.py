...
class PokerBotModel:
    ...
    def _go_to_next_street(self, game: Game, chat_id: ChatId, context: CallbackContext) -> None:
        ...
        """
        # --- بخش ۱: آماده‌سازی برای دور جدید ---
        # قبل از هر تصمیمی، وضعیت بازیکنان و میز رو برای دور شرط‌بندی بعدی آماده می‌کنیم.
        for p in game.players_by(states=(PlayerState.ACTIVE,)):
            p.has_acted = False
        ...
        # --- بخش ۲: مسیرهای خروج (پایان دست) ---
        # اینجا دو "درِ خروجی" اصلی برای پایان دادن به دست وجود داره.

        # خروجی ۱: همه فولد کرده‌اند و فقط یک نفر باقی مانده است.
        if len(contenders) <= 1:
            self._showdown(game, chat_id, context) # مستقیم به showdown می‌رویم تا برنده را اعلام کند.
            return # <-- تمام! از متد خارج شو و به بقیه کدها کاری نداشته باش.

        # خروجی ۲: دور شرط‌بندی "ریور" تمام شده و بیش از یک نفر باقی مانده است.
        if game.state == GameState.ROUND_RIVER:
            self._showdown(game, chat_id, context) # زمان نمایش کارت‌ها و مشخص شدن برنده است.
            return # <-- تمام! از متد خارج شو.

        # --- بخش ۳: مسیر ادامه بازی ---
        # اگر کد به اینجا رسیده، یعنی دست هنوز تمام نشده و باید به مرحله بعد برویم.
        ...
        # پس از رفتن به مرحله بعد، اولین نفر برای بازی رو مشخص و حلقه بازی رو دوباره فعال می‌کنیم.
