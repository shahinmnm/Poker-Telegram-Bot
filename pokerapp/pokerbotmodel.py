...
class PokerBotModel:
    ...
    async def _auto_start_countdown(
        ...
        """Run a simple countdown and start the game when it reaches zero."""
        remaining = context.chat_data.get("start_countdown", 60)
        try:
            while remaining > 0:
                keyboard_buttons = [[
                    InlineKeyboardButton(text="نشستن سر میز", callback_data="join_game"),
                    InlineKeyboardButton(
                        text=f"شروع بازی ({remaining})", callback_data="start_game"
                    ),
                ]]
                ...
                context.chat_data["start_countdown"] = remaining
    ...
    async def _schedule_auto_start(
        ...
        if context.chat_data.get("start_countdown_task"):
            return
        context.chat_data["start_countdown"] = 60
        task = asyncio.create_task(
            self._auto_start_countdown(context, game, chat_id)
        )
        context.chat_data["start_countdown_task"] = task

    def _cancel_auto_start(self, context: CallbackContext) -> None:
        task = context.chat_data.pop("start_countdown_task", None)
        if task:
            task.cancel()
